<?php
    ...
    $sql = "SELECT curso.nome_curso, curso.id_curso, turma.id_turma, turma.nome_turma, turma.codigo_oferta, turma.horario_inicio, turma.horario_termino, turma.data_inicio FROM curso INNER JOIN turma ON curso.id_curso = turma.id_curso WHERE curso.id_curso = ? ORDER BY curso.nome_curso";
    $envia = mysqli_prepare($conexao, $sql);
    mysqli_stmt_bind_param($envia, "i", $valorSelecionado);
    mysqli_stmt_execute($envia);
    mysqli_stmt_bind_result($envia, $nomeCurso, $idCurso, $idTurma, $nomeTurma, $codigoOferta, $horaInicio, $horaTermino, $dataInicio);
